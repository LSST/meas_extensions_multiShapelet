...
double callObjective(unsigned n, double const * x, double * gradient, void * func_data) {
    Objective * obj = reinterpret_cast<Objective>(func_data);
...
NLopt::NLopt(NLopt const & other) : 
    _obj(other._obj), _iqc(other._iqc), _eqc(other._eqc), _opt(::nlopt_copy(other))
{
    if (!_opt) {
        throw LSST_EXCEPT(
            pex::exceptions::RuntimeError,
...
std::pair<NLopt::ResultEnum> optimize(Eigen::VectorXd & x) {
...
void NLopt::setObjective(CONST_PTR(Objective) const & func, bool maximize) {
    ...
    _obj = obj;
    if (maximize) {
        ::nlopt_set_max_objective(_opt, callObjective, _obj.get());
    } else {
        ::nlopt_set_min_objective(_opt, callObjective, _obj.get());
    }
}
...
void NLopt::setEqualityConstraint(CONST_PTR(Constraint) const & eqc) {
    assert(iqc.getParameterSize() == getParameterSize());
    _eqc.push_back(eqc);
    ::nlopt_add_equality_constraint(_opt, callObjective, const_cast<Objective*>(eqc.get()), tolerance);
}
void NLopt::removeInequalityConstraint() {
    _iqc.clear();
    ::nlopt_remove_inequality_constraints(_opt);
}
void NLopt::removeEqualityConstraint() {
    _eqc.clear();
